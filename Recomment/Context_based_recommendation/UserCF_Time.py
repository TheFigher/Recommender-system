...
"""
    实现一个“增加时间衰减函数的协同过滤算法”    增加时间衰减函数的协同过滤算法实现 UserCF
"""
            ...
            for i, users in item_eval_by_users.items():
                for u in users:
                    ...
                    for v in users:
                        ...
                        if u == v:
                            continue
                        count[u][v] += 1 / (1 + self.alpha * abs(self.train[u][i]["time"] - self.train[v][i]["time"]) / (24 * 60 * 60)) * 1 / math.log(1 + len(users))
            # 构建相似度矩阵
            userSim = dict()
...
if __name__ == '__main__':
    cf = NewUserCF(r"C:\Users\administered\Desktop\推荐系统开发实战\数据\data\ml-1m\ratings.dat")
